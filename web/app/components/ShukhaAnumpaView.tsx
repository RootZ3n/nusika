...
/**
 * ShukhaAnumpaView — three-pane story workshop: drafts list (left), editor
 * (center), Peh's feedback (right). A tale, a fable, a remembered life.
 *
...
import type { ShukhaAnumpaDraft } from "../types";

const SA_ACCENT = "#a78bfa";

export interface ShukhaAnumpaViewProps {
  drafts: ShukhaAnumpaDraft[];
...
export function ShukhaAnumpaView(props: ShukhaAnumpaViewProps) {
  ...
  // Refresh drafts whenever the view mounts.
  useEffect(() => {
    void fetchDrafts();
  }, [fetchDrafts]);
  ...
  return (
    <div style={{ display: "flex", height: "100%", minHeight: 0, overflow: "hidden" }}>
      ...
      <div style={{ width: 220, flexShrink: 0, borderRight: "1px solid var(--border)", overflowY: "auto", padding: 12, display: "flex", flexDirection: "column", gap: 6 }}>
        <div style={{ ...mono, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: "var(--text-muted)", marginBottom: 4 }}>Tales</div>
        <button
          onClick={() => { setText(""); setTitle(""); setFeedback(""); }}
          style={{ ...mono, padding: "6px 10px", borderRadius: 6, border: `1px solid ${SA_ACCENT}30`, background: `${SA_ACCENT}10`, color: SA_ACCENT, cursor: "pointer", fontWeight: 700, textAlign: "left" }}
        >+ New Tale</button>
        {drafts.map((d) => (
        ...
        {drafts.length === 0 && (
          <div style={{ ...mono, color: "var(--text-muted)", padding: 8 }}>No tales yet. Peh remembers this one. Mostly.</div>
        )}
      </div>
        ...
        <div style={{ display: "flex", gap: 8, paddingTop: 12, borderTop: "1px solid var(--border)", flexShrink: 0 }}>
          <button
            ...
            style={{
              ...
              background: text.trim() ? `linear-gradient(135deg, ${SA_ACCENT}, #f472b6)` : "var(--bg-raised)",
          ...
          <button
            ...
            style={{
              ...
              border: `1px solid ${SA_ACCENT}30`,
              background: `${SA_ACCENT}08`,
              color: text.trim() ? SA_ACCENT : "var(--text-muted)",
              fontFamily: "var(--font-mono)", fontSize: 14, fontWeight: 700,
              cursor: text.trim() ? "pointer" : "default",
            }}
          >Save Tale</button>
      ...
      <div style={{ width: 320, flexShrink: 0, borderLeft: "1px solid var(--border)", overflowY: "auto", padding: 16, display: "flex", flexDirection: "column", gap: 12 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <div style={{ width: 28, height: 28, borderRadius: "50%", background: SA_ACCENT, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "var(--font-display)", fontSize: 14, fontWeight: 700, color: "#060810" }}>P</div>
          <div>
            <div style={{ fontFamily: "var(--font-display)", fontSize: 14, fontWeight: 700, color: SA_ACCENT }}>Peh</div>
            <div style={{ ...mono, color: "var(--text-muted)" }}>Story Guide</div>
          </div>
        </div>
        {feedbackLoading && (
          <div style={{ fontFamily: "var(--font-body)", fontSize: 14, color: "var(--text-muted)", fontStyle: "italic", lineHeight: 1.6 }}>
            Reading your tale carefully...
          </div>
        )}
        {!feedbackLoading && feedback && (
          <div style={{
            ...
            background: `${SA_ACCENT}08`, border: `1px solid ${SA_ACCENT}15`,
          }}>
            {feedback}
          </div>
        )}
        {!feedbackLoading && !feedback && (
          <div style={{ fontFamily: "var(--font-body)", fontSize: 14, color: "var(--text-muted)", lineHeight: 1.6, fontStyle: "italic" }}>
            Gather around. Write something and share it with me. I&apos;ll read every word.
          </div>
        )}
      </div>
    </div>
