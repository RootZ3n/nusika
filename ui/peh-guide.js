// ══════════════════════════════════════════════════════════════════════
// NUSIKA · PEH GUIDE — Peh's Journal (a running log of activity as you
// explore the Academy). Lives OUTSIDE #app so it survives full re-renders.
// Voice: patient librarian-scholar, warm and encouraging.
// ══════════════════════════════════════════════════════════════════════
(function () {
  'use strict';
  ...
  function renderList() {
    if (!listEl) return;
    if (!journal.length) {
      listEl.innerHTML = '<li class="peh-jrnl-empty">The Academy is quiet. Tap a location on the map to begin exploring.</li>';
      return;
    }
  ...
  function init() {
    ...
    btn.type = 'button';
    btn.title = "Peh's Journal — Academy activity log";
    ...
    drawer.innerHTML =
      '<header class="peh-jrnl-head"><b>Peh\'s Journal</b><span>notes from the Academy halls</span>' +
  ...
  window.NusGuide = { init: init, log: log, toggle: toggle, entries: function () { return journal.slice(); } };
})();
