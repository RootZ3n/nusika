...
test("magister-api unit binds API to 127.0.0.1:18793 only", () => {
  const text = readUnit("magister-api.service");
  assert.match(text, /Environment=MAGISTER_HOST=127\.0\.0\.1/, "API host must be loopback");
  assert.match(text, /Environment=MAGISTER_PORT=18793/, "API port must be 18793");
  assert.match(text, /Environment=MAGISTER_ALLOW_PUBLIC_BIND=false/, "public bind guard must be off");
  assert.doesNotMatch(text, /MAGISTER_HOST=0\.0\.0\.0/, "API must never set HOST=0.0.0.0");
});

test("magister-kokoro unit binds Kokoro to 127.0.0.1:18794 only", () => {
  const text = readUnit("magister-kokoro.service");
  assert.match(text, /--host 127\.0\.0\.1/, "Kokoro must bind to loopback");
  assert.match(text, /--port 18794/, "Kokoro port must be 18794");
  assert.match(text, /MAGISTER_KOKORO_HOST=127\.0\.0\.1/, "Kokoro env host must be loopback");
  assert.match(text, /MAGISTER_KOKORO_PORT=18794/, "Kokoro env port must be 18794");
  assert.doesNotMatch(text, /--host 0\.0\.0\.0/, "Kokoro must never bind 0.0.0.0");
});

test("magister-web unit exposes Next on 0.0.0.0:3003 and proxies to loopback API", () => {
  const text = readUnit("magister-web.service");
  assert.match(text, /-p 3003/, "Web port must be 3003");
  assert.match(text, /-H 0\.0\.0\.0/, "Web is the public surface; expected 0.0.0.0");
  assert.match(text, /MAGISTER_API_URL=http:\/\/127\.0\.0\.1:18793/, "Web must proxy to loopback API");
});

test("magister.target Requires API and Wants Web/Kokoro (truth model)", () => {
  const text = readUnit("magister.target");
  ...
  assert.match(text, /Requires=.*magister-api\.service/, "target must Require API for truth tracking");
  // Web + Kokoro are useful on their own; Wants= so a partial stack still
  // surfaces a degraded banner instead of cascade-stopping.
  assert.match(text, /Wants=.*magister-web\.service/, "target must Want Web");
  assert.match(text, /Wants=.*magister-kokoro\.service/, "target must Want Kokoro");
  // Explicitly forbid API in Wants= — that's the prior buggy setup.
  assert.doesNotMatch(
    text,
    /^Wants=[^\n]*magister-api\.service/m,
    "target must NOT have API in Wants= — switched to Requires= on 2026-05-22",
  );
});

test("magister-api unit uses Restart=always (load-bearing service)", () => {
  const text = readUnit("magister-api.service");
